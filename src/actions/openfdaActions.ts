...
export const searchDrugs = async (data: { search: string; options?: any }) => {
  try {
    const session = await auth();
    const accessToken = (session as any)?.accessToken;
    if (!accessToken) return { success: false, message: 'Unauthorized', statusCode: 401 };

    const response = await fetch(`${API_URL}/openfda/drug/search`, {
      method: 'POST',
...
export const searchAdverseEvents = async (data: { search: string; options?: any }) => {
  try {
    const session = await auth();
    const accessToken = (session as any)?.accessToken;
    if (!accessToken) return { success: false, message: 'Unauthorized', statusCode: 401 };

    const response = await fetch(`${API_URL}/openfda/drug/events`, {
      method: 'POST',
      headers: getHeaders(accessToken),
      body: JSON.stringify(data),
    });
