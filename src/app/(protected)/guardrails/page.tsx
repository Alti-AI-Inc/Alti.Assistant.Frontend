...
const LEGACY_KEY = 'aphura_custom_guardrails';
...
export default function GuardrailsPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        ...
        // Migrate legacy guardrails if they exist
        const legacy = localStorage.getItem(LEGACY_KEY);
        if (legacy && legacy.trim()) {
          const migrated: Guardrail[] = [{
            id: Date.now().toString(),
            text: legacy,
            createdAt: Date.now()
          }];
          setGuardrails(migrated);
  ...
  const handleDelete = (id: string) => {
    saveGuardrails(guardrails.filter(i => i.id !== id));
    toast.success('Guardrail removed');
  };

  const filteredGuardrails = guardrails.filter(i => 
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#e1e1e1] dark:bg-zinc-950 overflow-hidden">
      ...
      <div className="mx-auto w-full flex-1 px-8 py-6 flex flex-col overflow-y-auto">
        ...
        <IosConfirmModal
          ...
          title="Delete Guardrail"
          description="Are you sure you want to delete this guardrail?"
          confirmText="Delete"
