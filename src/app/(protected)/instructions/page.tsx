...
const InstructionsContent = () => {
  ...
  useEffect(() => {
    setIsMounted(true);
    const storedInst = localStorage.getItem('insosearch_instructions');
    if (storedInst) {
      ...
      localStorage.setItem('insosearch_instructions', JSON.stringify(defaults));
    }
  }, []);
  ...
  const handleAddInstruction = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('insosearch_instructions', JSON.stringify(updatedList));
      setInputVal('');
      setIsSaving(false);
    }, 300);
  };
