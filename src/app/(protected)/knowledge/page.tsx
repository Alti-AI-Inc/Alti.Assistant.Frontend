...
import { documentsDummyData } from '@/lib/dummy-documents-data';
import { useModalStore } from '@/stores/useModalStore';
import jsPDF from 'jspdf';
import { ArrowLeft, File, Trash, X } from 'lucide-react';
      ...
      <div className="flex items-center justify-between">
        <div className="mb-6 flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-gray-900">
            {history.length > 0 ? folderName : 'Knowledge Bots'}
          </h1>
        </div>
        {history.length > 0 && (
          <div>
      ...
      {isModelOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 flex w-full max-w-md flex-col gap-3 rounded-xl bg-gray-50 p-4 shadow-lg">
            ...
            <h2 className="mt-2 text-base font-medium">Create Folder</h2>
            <Input placeholder="Folder Name" />
            <div className="mt-3 flex justify-end gap-2">
              <Button
                onClick={() => setIsModelOpen(false)}
                className="border-[1px] border-black bg-white text-black hover:bg-white/90"
              >
                Cancel
              </Button>
              <Button
