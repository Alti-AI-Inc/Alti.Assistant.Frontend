...
# Set active project
gcloud config set project insosearch-assistant-prod
...
```bash
gcloud artifacts repositories create insosearch-assistant \
...
```bash
echo -n "your-auth-secret" | gcloud secrets create AUTH_SECRET --data-file=- --project=insosearch-assistant-prod

echo -n "pk_test_..." | gcloud secrets create NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY --data-file=- --project=insosearch-assistant-prod
```

To update an existing secret version:

```bash
echo -n "new-value" | gcloud secrets versions add AUTH_SECRET --data-file=- --project=insosearch-assistant-prod
```
...
gcloud projects add-iam-policy-binding insosearch-assistant-prod \
...
docker build `
  ...
  -t insosearch-assistant-frontend:local .
...
docker run --rm -p 3000:8080 `
  ...
  insosearch-assistant-frontend:local
```
...
```powershell
docker build --build-arg NEXT_PUBLIC_API_URL=https://apiv2.asonai.com/api/v1 --build-arg NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_... -t us-central1-docker.pkg.dev/insosearch-assistant-prod/insosearch-assistant/insosearch-assistant-frontend:latest .
```

### Step 3 — Push to Artifact Registry

```powershell
docker push us-central1-docker.pkg.dev/insosearch-assistant-prod/insosearch-assistant/insosearch-assistant-frontend:latest
```

### Step 4 — Deploy to Cloud Run

```powershell
gcloud run deploy insosearch-assistant-frontend `
  --image us-central1-docker.pkg.dev/insosearch-assistant-prod/insosearch-assistant/insosearch-assistant-frontend:latest `
  --region us-central1 `
  --platform managed `
...
**Service URL:**
https://insosearch-assistant-frontend-366561755636.us-central1.run.app
...
gcloud run services logs read insosearch-assistant-frontend --region us-central1 --project insosearch-assistant-prod --limit 50

# Check running service details
gcloud run services describe insosearch-assistant-frontend --region us-central1 --project insosearch-assistant-prod

# List all revisions
gcloud run revisions list --service insosearch-assistant-frontend --region us-central1 --project insosearch-assistant-prod

# Roll back to previous revision
gcloud run services update-traffic insosearch-assistant-frontend `
  --to-revisions REVISION_NAME=100 `
  --region us-central1 `
  --project insosearch-assistant-prod

# Stop local container
docker stop $(docker ps -q --filter ancestor=insosearch-assistant-frontend:local)
```
