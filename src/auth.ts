...
const verifyBackendJwt = (token: string): jwt.JwtPayload | null => {
  ...
  if (!secret) {
    console.error(
      'Aphura Auth: Neither JWT_ACCESS_TOKEN nor AUTH_SECRET is defined. Cannot verify backend JWT signature.'
    );
    return null;
  }
  ...
  try {
    ...
    console.error('Aphura Auth: Failed to verify backend JWT signature:', error);
    return null;
  }
};
