...
const verifyBackendJwt = (token: string): jwt.JwtPayload | null => {
  ...
  if (!secret) {
    console.error(
      'Inso Search Auth: Neither JWT_ACCESS_TOKEN nor AUTH_SECRET is defined. Cannot verify backend JWT signature.'
    );
    return null;
  }
  ...
  try {
    ...
    console.error('Inso Search Auth: Failed to verify backend JWT signature:', error);
    return null;
  }
};
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          const apiUrl = process.env.API_URL || process.env.NEXT_PUBLIC_API_URL || 'https://api.insosearch.com/api/v1';
