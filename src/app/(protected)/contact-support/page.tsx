...
const SupportContent = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const now = new Date();
      const userEmail = session?.user?.email || 'user@insohq.com';
      const newRequest = {
      ...
      const existing = localStorage.getItem('inso_support_requests');
      const requests = existing ? JSON.parse(existing) : [];
      requests.unshift(newRequest);
      localStorage.setItem('inso_support_requests', JSON.stringify(requests));
