...
export default function TaskHistoryLogs({
  ...
  useEffect(() => {
    ...
    if (taskName.includes('Report') || taskName.includes('GCP')) {
      lines.push(
        `[${new Date().toISOString()}] [fetch] GET secure connection to https://monitoring.cloud.insohq.com/v3...`,
        `[${new Date().toISOString()}] [status] 200 OK - connection established with Inso Assistant Cloud Monitoring API`,
