...
export const getPopulationData = async (data: { year?: string; state?: string; county?: string }) => {
  try {
    const session = await auth();
    const accessToken = (session as any)?.accessToken;
    if (!accessToken) return { success: false, message: 'Unauthorized', statusCode: 401 };

    const response = await fetch(`${API_URL}/census/population`, {
      method: 'POST',
...
export const getEconomicData = async (data: { year?: string; state?: string }) => {
  try {
    const session = await auth();
    const accessToken = (session as any)?.accessToken;
    if (!accessToken) return { success: false, message: 'Unauthorized', statusCode: 401 };

    const response = await fetch(`${API_URL}/census/economy`, {
      method: 'POST',
      headers: getHeaders(accessToken),
      body: JSON.stringify(data),
    });
