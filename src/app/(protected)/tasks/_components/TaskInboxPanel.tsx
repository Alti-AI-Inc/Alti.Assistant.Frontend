    ...
    <div className={cn(
      ...
      {isOpen && (
        <div className="h-[52px] flex items-center border-b border-black/10 dark:border-zinc-800/80 px-4 bg-[#FFFFFF] dark:bg-zinc-900 transition-all duration-300 flex-none">
          <div className="flex h-8 w-full items-center gap-2 rounded-lg border border-black/10 dark:border-zinc-800/80 bg-[#e1e1e1] dark:bg-zinc-800/50 px-3 shadow-xs transition-all focus-within:ring-1 focus-within:ring-black/20 dark:focus-within:ring-white/10">
      ...
      <div className={cn(
        ...
        {filteredRuns.map((run) => (
          <div 
            key={run.id}
            className="bg-[#e1e1e1] dark:bg-zinc-950/60 rounded-xl p-3.5 space-y-2.5 relative overflow-hidden"
          >
            {run.status === 'running' && (
        ...
        {filteredRuns.length === 0 && (
          <div className="flex flex-col items-center justify-center text-center p-8 border border-dashed border-black/10 dark:border-white/10 rounded-2xl min-h-[200px] bg-[#e1e1e1] dark:bg-zinc-800/20">
