...
import { CircleUserRound, LogOut, ReceiptText, Settings, Shield, Brain, SlidersHorizontal, ShieldAlert, UserPlus, Mail, Database, KeyRound, Scale, Users, CreditCard, FileText, LayoutGrid, Plus, Sparkles } from 'lucide-react';
  ...
  return (
    <>
      <div className="fixed left-0 top-0 h-screen w-64 bg-white dark:bg-zinc-900 border-r border-black/10 dark:border-zinc-800/80 flex flex-col pt-4 pb-2">
        ...
        <div
          className={cn(
            'bg-white dark:bg-zinc-900 sticky bottom-0 z-30 flex h-[64px] items-center justify-center p-4 py-2.5 border-t border-black/10 dark:border-zinc-800/80 w-full',
          )}
        >
          {!isLoggedIn ? (
                ...
                <DropdownMenuGroup>
                  ...
                  {!isSuperAdmin && (
                    <DropdownMenuItem onClick={() => router.push('/invite-friends')}>
                      <UserPlus className="text-black dark:text-white" /> Refer A Friend
                    </DropdownMenuItem>
                  )}
                  {!isSuperAdmin && (
