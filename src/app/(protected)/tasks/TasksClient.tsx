...
export default function TasksClient() {
  ...
  useEffect(() => {
    ...
    if (savedTasks) {
      ...
      localStorage.setItem('insosearch_automations', JSON.stringify(defaultTasks));
    }
    ...
    if (savedRuns) {
      try {
        ...
        if (parsed.length === 0 || hasOldFormat) {
          ...
          setRuns(defaultRuns);
          localStorage.setItem('insosearch_task_runs', JSON.stringify(defaultRuns));
        } else {
          setRuns(parsed);
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('insosearch_new_task_click', handleNewTaskClick);
    return () => {
      window.removeEventListener('insosearch_new_task_click', handleNewTaskClick);
    };
  }, []);
  ...
  const saveTasks = (newTasks: AutomationTask[]) => {
    setTasks(newTasks);
    localStorage.setItem('insosearch_automations', JSON.stringify(newTasks));
    window.dispatchEvent(new Event('insosearch_automations_updated'));
  };

  const saveRuns = (newRuns: TaskRun[]) => {
    setRuns(newRuns);
    localStorage.setItem('insosearch_task_runs', JSON.stringify(newRuns));
  };
  ...
  const handleCreateTask = () => {
    ...
    setTimeout(() => {
      ...
      setRuns(prevRuns => {
        ...
        localStorage.setItem('insosearch_task_runs', JSON.stringify(updated));
        return updated;
      });
  ...
  return (
    <div className="flex h-full w-full bg-[#e1e1e1] dark:bg-zinc-950 flex-row overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-8 relative overflow-hidden h-full">
        ...
        {/* Creator Form */}
        <div className="w-full max-w-[796px] flex flex-col items-center z-10 md:-translate-y-16">
          {isSpacesPage && (
