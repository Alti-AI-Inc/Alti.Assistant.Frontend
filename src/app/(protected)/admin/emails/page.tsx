...
export default function AdminEmailsPage() {
  const { emails, setEmails } = useAdminStore();
  ...
  const emailsList: EmailAccountEntry[] = emails
    ? emails.split('\n\n').filter(Boolean).map(item => {
        try {
          return JSON.parse(item);
        } catch (e) {
          return { name: item, email: '', password: '' };
        }
      })
    : [];
  ...
  const handleCreate = () => {
    ...
    const newEntry: EmailAccountEntry = {
      name: newAccount.trim(),
      email: newEmail.trim(),
      password: newPassword,
    };
    const newList = [newEntry, ...emailsList];
    setEmails(newList.map(item => JSON.stringify(item)).join('\n\n'));
    setNewAccount('');
    setNewEmail('');
    setNewPassword('');
  ...
  const handleStartEdit = (index: number, val: EmailAccountEntry) => {
  ...
  const handleSaveEdit = () => {
    ...
    const newList = emailsList.map((item, idx) =>
      idx === editingIndex
        ? { name: editValue.trim(), email: editEmail.trim(), password: editPassword }
        : item,
    );
    setEmails(newList.map(item => JSON.stringify(item)).join('\n\n'));
  ...
  const handleDelete = () => {
    ...
    const newList = emailsList.filter((_, idx) => idx !== deletingIndex);
    setEmails(newList.map(item => JSON.stringify(item)).join('\n\n'));
    setDeletingIndex(null);
  };

  const filteredEmails = emailsList
                ...
                <DialogContent className="max-w-lg rounded-[20px] bg-[#F5F5F7] dark:bg-zinc-900 border-none shadow-xl animate-in fade-in-50 duration-150">
                  <DialogHeader>
                    ...
                    <DialogDescription className="text-gray-500 dark:text-gray-400 text-sm">
                      Enter the name and password details for the new email account below.
                    </DialogDescription>
                  </DialogHeader>
                    ...
                    <div>
                      <div className="relative">
                        <Input
                          ...
                          autoComplete="new-password"
                          onKeyDown={e => {
                            if (e.key === 'Enter' && newAccount.trim() && newEmail.trim() && newPassword.trim()) {
                              e.preventDefault();
                              handleCreate();
                            }
                          }}
                  ...
                  <DialogFooter>
                    ...
                    <Button
                      ...
                      disabled={!newAccount.trim() || !newEmail.trim() || !newPassword.trim()}
                      className={(!newAccount.trim() || !newEmail.trim() || !newPassword.trim())
              ...
              {filteredEmails.length === 0 ? null : (
                <div className="flex flex-col gap-2 pb-4">
                  {filteredEmails.map(({ item, index }) => (
                    ...
                    >
                      {/* Left Icon (Mail) & Content column merged */}
                        ...
                        >
                          ...
                          <DialogContent className="max-w-lg rounded-[20px] bg-[#F5F5F7] dark:bg-zinc-900 border-none shadow-xl animate-in fade-in-50 duration-150">
                            <DialogHeader>
                              ...
                              <DialogDescription className="text-gray-500 dark:text-gray-400 text-sm">
                                Make changes to the email account details below.
                              </DialogDescription>
                            </DialogHeader>
                            <div className="py-4 flex flex-col gap-3">
                              <div>
                              ...
                              <div>
                                <div className="relative">
                                  <Input
                                    ...
                                    autoComplete="new-password"
                                    onKeyDown={e => {
                                      if (e.key === 'Enter' && editValue.trim() && editEmail.trim() && editPassword.trim()) {
                                        e.preventDefault();
                                        handleSaveEdit();
                                      }
                                    }}
                            ...
                            <DialogFooter>
                              ...
                              <Button
                                ...
                                disabled={!editValue.trim() || !editEmail.trim() || !editPassword.trim()}
                                className={(!editValue.trim() || !editEmail.trim() || !editPassword.trim())
                                  ? "bg-zinc-200 text-zinc-400 dark:bg-zinc-800 dark:text-zinc-500 cursor-not-allowed"
                                  : "bg-black hover:bg-black/90 text-white dark:bg-white dark:text-black dark:hover:bg-white/95"
                                }
                              >
