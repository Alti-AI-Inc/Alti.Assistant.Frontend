...
const GuardrailsContent = () => {
  ...
  useEffect(() => {
    setIsMounted(true);
    const storedGuard = localStorage.getItem('insosearch_guardrails');
    if (storedGuard) {
      ...
      localStorage.setItem('insosearch_guardrails', JSON.stringify(defaults));
    }
  }, []);
  ...
  const handleAddGuardrail = () => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('insosearch_guardrails', JSON.stringify(updatedList));
      setInputVal('');
      setIsSaving(false);
    }, 300);
  };
