...
This guide covers everything needed to build, test, and deploy the Inso Assistant
...
# Set active project
gcloud config set project inso-assistant-prod
...
```bash
gcloud artifacts repositories create inso-assistant \
...
```bash
echo -n "your-auth-secret" | gcloud secrets create AUTH_SECRET --data-file=- --project=inso-assistant-prod

echo -n "pk_test_..." | gcloud secrets create NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY --data-file=- --project=inso-assistant-prod
```

To update an existing secret version:

```bash
echo -n "new-value" | gcloud secrets versions add AUTH_SECRET --data-file=- --project=inso-assistant-prod
```
...
gcloud projects add-iam-policy-binding inso-assistant-prod \
...
gcloud iam workload-identity-pools providers create-oidc github-provider \
  ...
  --attribute-condition="assertion.repository=='Inso-AI-Inc/Inso.Assistant.Frontend'" \
  --project=inso-assistant-prod
...
gcloud iam service-accounts add-iam-policy-binding \
  ...
  --member="principalSet://iam.googleapis.com/projects/366561755636/locations/global/workloadIdentityPools/github-pool/attribute.repository/Inso-AI-Inc/Inso.Assistant.Frontend" \
  --project=inso-assistant-prod \
  --condition=None
```
...
docker build `
  ...
  -t inso-assistant-frontend:local .
...
```powershell
docker build --build-arg NEXT_PUBLIC_API_URL=https://apiv2.asonai.com/api/v1 --build-arg NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_test_... -t us-central1-docker.pkg.dev/inso-assistant-prod/inso-assistant/inso-assistant-frontend:latest .
```

### Step 3 — Push to Artifact Registry

```powershell
docker push us-central1-docker.pkg.dev/inso-assistant-prod/inso-assistant/inso-assistant-frontend:latest
```

### Step 4 — Deploy to Cloud Run

```powershell
gcloud run deploy inso-assistant-frontend `
  --image us-central1-docker.pkg.dev/inso-assistant-prod/inso-assistant/inso-assistant-frontend:latest `
  --region us-central1 `
  --platform managed `
...
**Service URL:**
https://inso-assistant-frontend-366561755636.us-central1.run.app
...
gcloud run services logs read inso-assistant-frontend --region us-central1 --project inso-assistant-prod --limit 50

# Check running service details
gcloud run services describe inso-assistant-frontend --region us-central1 --project inso-assistant-prod

# List all revisions
gcloud run revisions list --service inso-assistant-frontend --region us-central1 --project inso-assistant-prod

# Roll back to previous revision
gcloud run services update-traffic inso-assistant-frontend `
  --to-revisions REVISION_NAME=100 `
  --region us-central1 `
  --project inso-assistant-prod

# Stop local container
docker stop $(docker ps -q --filter ancestor=inso-assistant-frontend:local)
```
